<?php
/**
 * Code is released under the GNU LGPL
 *
...
class ClassLoader
{
     ...
     * @var array
     */
    private $_skipNames = array(); // WAJ added
     ...
     * @var string
     */
    private $_filesRegex = '/\.php$/'; // WAJ changed
     ...
     */
    public function setCacheFile($pathToFile)
    {
        $this->_cacheFile = $pathToFile;
        if ($this->_cacheFile && file_exists($this->_cacheFile))
        {
            $this->_classes = include($this->_cacheFile);
        }
    }
     ...
     */
    public function addLibraryFolder($folder)
    {
        if ($realpath = realpath($folder) and is_dir($realpath))
        {
            $this->_folders[] = $realpath;
        }
        else
        {
            throw new Exception('Failed to open dir : ' . $folder);
        }
    }

    /**
     * Synonym for addLibraryFolder()
     */
    public function addBaseFolder($folder) // WAJ added
    {
      $this->addLibraryFolder($folder);
    }

    /**
     * Exclude a folder from parsing
     *
     * @param string $path Folder to exclude
     */
    public function skipFolder($path)
    {
        if ($realpath = realpath($path) and is_dir($realpath))
        {
            $this->_skipFolders[] = $realpath . DIRECTORY_SEPARATOR;
        }
        else
        {
    ...
    /**
     * Exclude from parsing if a certain string is found in name
     *
     * @param string $path Folder to exclude
     */
    public function skipAnyNameContaining($string) // WAJ added
    {
        $this->_skipNames[] = $string;
    }
    ...
    private function parseFolder($folder)
    {
        ...
        foreach ($files as $file)
        {
            ...
            {
                foreach ($this->_skipFolders as $folder)
                {
                    $len = strlen($folder);
                ...
                // var_dump( $this->_skipNames );

                foreach ($this->_skipNames as $folderName) // WAJ added
                {
                    // d( $file->getPathname() );
                    // d( $folder );
                    if (false !== strpos( $file->getPathname(), $folderName))
                    {
                        continue 2;
                    }                   
                }
